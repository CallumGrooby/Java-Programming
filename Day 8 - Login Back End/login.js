...
// http://localhost:3000/
app.get('/', function(request, response) { //Gets sends the html document to the local host
...
app.get('/home', function(request, response) {
    ...
    if (request.session.loggedin) {
      ...
      // Not logged in
      response.send('Please login or create an account to view this page!');
    }
    response.end();
  });
  ...
  app.post('/create_account', function(request, response) {
    ...
    if (username && email && password) {
        ...
        connection.query('SELECT * FROM accounts WHERE email = ?', [email], function(error, results, fields) {
            ...
            if (results.length > 0) {
                // Add the error message to the response as an HTML element
                let errorMessage = '<div class="error-message">Email already in use!</div>';
                response.send(errorMessage);
                response.end();
            } else {
                // If the email is not in use, insert the account into the database
